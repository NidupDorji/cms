...
if ($stmt = $conn->prepare("SELECT course_title, course_description, thumbnail_path FROM courses WHERE course_id = ?")) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $oldCourseDir = '../teacher/courses/' . $courseTitle;
  $newCourseDir = '../teacher/courses/' . $newTitle;
  ...
  if ($stmt = $conn->prepare("UPDATE courses SET course_title = ?, course_description = ? WHERE course_id = ?")) {
    $stmt->bind_param("ssi", $newTitle, $newDescription, $courseId);
  ...
  $courseDir = '../teacher/courses/' . $courseTitle;
